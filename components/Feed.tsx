...
const CATEGORIES = ['All', 'Facilities', 'Events', 'Academics', 'Social News', 'Topics', 'Announcements']
...
export default function Feed({ initialTips }: { initialTips: any[] }) {
  ...
  const filtered = activeCategory === 'All'
    ? initialTips
    : initialTips.filter(t => t.category === activeCategory)
...
function TipCard({ tip, index }: { tip: any; index: number }) {
  ...
  // Fetch comment count immediately on mount
  useEffect(() => {
    fetch(`/api/comments?tip_id=${tip.id}`)
      .then(r => r.json())
      .then(json => setCommentCount((json.comments || []).length))
      .catch(() => {})
  }, [tip.id])
  ...
  return (
    <div ref={cardRef} className="card" style={{ animation: `fadeIn 0.4s ease ${index * 0.05}s both` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span className="seal">
          {isEditorPost ? '✓ Posted by editor' : '✓ Verified by editor'}
        </span>
      </div>
